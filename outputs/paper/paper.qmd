...
state_map <- data.frame(state.name = tolower(state.name), state.abb = state.abb)
counties$region <- state_map$state.abb[match(counties$region, state_map$state.name)]
counties$subregion <- tools::toTitleCase(counties$subregion)

mer <- merge(counties, data, by.x=c("subregion", "region"), by.y=c("county", "state"), all.x = TRUE)
mer$death_rate[is.na(mer$death_rate)] <- 0

ggplot(data=mer, mapping=aes(x=long, y=lat, group=group, fill=death_rate)) + 
  geom_polygon(color="black") +
  scale_fill_gradient(low="white", high="blue", name="Income") + 
  ggtitle('U.S. Map with Income by County') + 
  theme_minimal() + 
  theme(axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank(),
        axis.title.y=element_blank(), axis.text.y=element_blank(), axis.ticks.y=element_blank()) +
  coord_fixed(1.3)

ggplot(mer, aes(x=long, y=lat, group=group)) + 
  geom_polygon(color="black", fill="gray") +
  coord_map() +
  theme_minimal()

duplicated_rows <- data[duplicated(data[c("county", "state")]),]
